...
import net.fabricmc.loader.api.ModDependency;

public class FabricRuleBreakAll extends FabricRuleBreak {

	final FabricRuleBreakOnly[] options;
	final ModDependency.All publicDep;

	public FabricRuleBreakAll(Logger logger, RuleContext ctx, LoadOption option, ModDependency.All all) {

		super(option);
		this.publicDep = all;
		List<FabricRuleBreakOnly> optionList = new ArrayList<>();

		for (ModDependency.Only only : all) {
			if (!only.shouldIgnore()) {
				FabricModDepOption sub = new FabricModDepOption(only);
				ctx.addOption(sub);
				FabricRuleBreakOnly dep = new FabricRuleBreakOnly(logger, ctx, sub, only);
				ctx.addRule(dep);
				optionList.add(dep);
			}
		}

		this.options = optionList.toArray(new FabricRuleBreakOnly[0]);
	}
	...
	boolean hasAnyConflictingOptions() {
		for (FabricRuleBreakOnly on : options) {

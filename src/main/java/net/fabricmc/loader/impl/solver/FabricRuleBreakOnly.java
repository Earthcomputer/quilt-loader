...
class FabricRuleBreakOnly extends FabricRuleBreak {
	final Logger logger;
	...
	final FabricRuleDep unless;

	public FabricRuleBreakOnly(Logger logger, RuleContext ctx, LoadOption source, ModDependency.Only publicDep) {
		super(source);
		this.logger = logger;
		...
		if (except != null && !except.shouldIgnore()) {
			FabricModDepOption option = new FabricModDepOption(except);

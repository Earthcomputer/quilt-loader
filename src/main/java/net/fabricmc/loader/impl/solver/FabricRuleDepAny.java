...
public class FabricRuleDepAny extends FabricRuleDep {

	final FabricRuleDepOnly[] options;
	final ModDependency.Any publicDep;

	public FabricRuleDepAny(Logger logger, RuleContext ctx, LoadOption option, ModDependency.Any any) {

		super(option);
		this.publicDep = any;
		List<FabricRuleDepOnly> optionList = new ArrayList<>();

		for (ModDependency.Only only : any) {
			if (!only.shouldIgnore()) {
				FabricModDepOption sub = new FabricModDepOption(only);
				ctx.addOption(sub);
				FabricRuleDepOnly dep = new FabricRuleDepOnly(logger, ctx, sub, only);
				ctx.addRule(dep);
				optionList.add(dep);
			}
		}

		this.options = optionList.toArray(new FabricRuleDepOnly[0]);
	}
	...
	boolean hasAnyValidOptions() {
		for (FabricRuleDepOnly on : options) {

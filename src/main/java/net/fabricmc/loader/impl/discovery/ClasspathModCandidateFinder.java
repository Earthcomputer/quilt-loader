...
public class ClasspathModCandidateFinder implements ModCandidateFinder {
					...
					if (file.exists() && file.isDirectory()) {
						try {
							...
							if (!modsList.contains(url)) {
								...
								if (!FabricLoaderImpl.INSTANCE.isDevelopmentEnvironment() || !url.equals(fabricCodeSource)) {
	...
	protected void addModSources(FabricLoaderImpl loader, Set<URL> modsList, String name) throws IOException {
		Enumeration<URL> mods = FabricLauncherBase.getLauncher().getTargetClassLoader().getResources(name);

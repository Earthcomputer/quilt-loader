...
import net.fabricmc.loader.impl.entrypoint.FabricEntrypointException;
...
class EntrypointStorage {
	...
	private static class OldEntry implements Entry {
		private static final net.fabricmc.loader.impl.language.LanguageAdapter.Options options = net.fabricmc.loader.impl.language.LanguageAdapter.Options.Builder.create()
			.missingSuperclassBehaviour(net.fabricmc.loader.impl.language.LanguageAdapter.MissingSuperclassBehavior.RETURN_NULL)
			.build();
		...
		public <T> T getOrCreate(Class<T> type) throws Exception {
			if (object == null) {
				net.fabricmc.loader.impl.language.LanguageAdapter adapter = (net.fabricmc.loader.impl.language.LanguageAdapter) Class.forName(languageAdapter, true, FabricLauncherBase.getLauncher().getTargetClassLoader()).getConstructor().newInstance();
	...
	protected void addDeprecated(ModContainer modContainer, String adapter, String value) throws ClassNotFoundException, LanguageAdapterException {
		FabricLoaderImpl.INSTANCE.getLogger().debug("Registering 0.3.x old-style initializer " + value + " for mod " + modContainer.metadata().id());
	...
	protected void add(ModContainer modContainer, String key, AdapterLoadableClassEntry metadata, Map<String, LanguageAdapter> adapterMap) throws Exception {
		...
		FabricLoaderImpl.INSTANCE.getLogger().debug("Registering new-style initializer " + metadata.getValue() + " for mod " +  modContainer.metadata().id() + " (key " + key + ")");
	...
	protected <T> List<T> getEntrypoints(String key, Class<T> type) {
		...
		FabricEntrypointException exception = null;
		...
		for (Entry entry : entries) {
			try {
				...
				if (exception == null) {
					exception = new FabricEntrypointException(key, entry.getModContainer().metadata().id(), t);
				} else {
		...
		for (Entry entry : entries) {
			results.add(new EntrypointContainerImpl<>(entry.getModContainer(), () -> {
				try {
					...
					throw new FabricEntrypointException(key, entry.getModContainer().metadata().id(), ex);
				}
			}));
		}
